...
// an ETag, each optionally negated, with its own lexer. x/net has a parser for
// it and keeps it unexported, and a lock is the one place where a parser bug is
// a silent authorisation failure rather than a visible one. So this is copied
// rather than written: it arrives with 322 lines of the Go project's own tests,
// which is the whole reason.
//
// **Copied, not adapted.** The only change from upstream is the package clause,
// so a later diff against x/net is a diff and not an argument.
//
// What it costs, said here rather than discovered: this is Go source and not a
// vendored asset, so deps.allow cannot see it, no upstream fix will arrive on
// its own, and it is ours to maintain from now on. See CLAUDE.md.
