...
const (
	...
	trackPrune  = `DELETE FROM track_annotations
		WHERE file_id = $1 AND owner_id = $2 AND starred_at IS NULL AND rating = 0 AND play_count = 0`
	// GREATEST ignores a NULL, so the first play needs no COALESCE.
	trackPlay = `INSERT INTO track_annotations (file_id, owner_id, starred_at, rating, play_count, played_at)
		...
		ON CONFLICT (file_id, owner_id) DO UPDATE SET
			...
			played_at = GREATEST(track_annotations.played_at, excluded.played_at)`
...
func (r *repo) RecordPlay(ctx context.Context, owner string, fileID int64, at time.Time) error {
	...
	if _, err := r.q.ExecContext(ctx, trackPlay, fileID, owner, at); err != nil {
...
func (r *repo) AnnotationsOf(ctx context.Context, owner string, subjects []db.Subject) (map[db.Subject]db.Annotation, error) {
	...
	if albums > 0 {
		...
		for i := range albums {
			...
			n := 3 + 2*i
			fmt.Fprintf(&match, "(m.album_artist = $%d AND m.album = $%d)", n, n+1)
		}
		query := `SELECT m.album_artist, m.album, SUM(t.play_count), MAX(t.played_at)
			...
			WHERE f.owner_id = $1 AND m.kind = $2 AND t.play_count > 0
			  AND (` + match.String() + `)
...
func scanTrackAnnotation(rows *sql.Rows) (annotationRow, error) {
	var id int64
	var starred, played sql.NullTime
	...
	if played.Valid {
		row.annotation.Played = played.Time.UTC()
	}
	return row, nil
}
...
func scanAlbumPlays(rows *sql.Rows) (annotationRow, error) {
	...
	row.annotation.Played = played.UTC()
	return row, nil
}
