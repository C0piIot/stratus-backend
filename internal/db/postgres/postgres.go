...
func (r *repo) ListFiles(ctx context.Context, owner, dir string) ([]db.File, error) {
	...
	for rows.Next() {
		var f db.File
		if err := rows.Scan(&f.ID, &f.OwnerID, &f.Path, &f.BlobKey, &f.Size, &f.MTime, &f.ETag, &f.MIMEType); err != nil {
...
func (r *repo) MoveFile(ctx context.Context, owner, from, to string) error {
	...
	const query = `UPDATE files SET path = $1, parent_path = $2 WHERE owner_id = $3 AND path = $4`
...
func (r *repo) DeleteFile(ctx context.Context, owner, path string) error {
	...
	result, err := r.q.ExecContext(ctx, `DELETE FROM files WHERE owner_id = $1 AND path = $2`, owner, path)
	if err != nil {
...
func scanFile(row *sql.Row) (db.File, error) {
	var f db.File
	if err := row.Scan(&f.ID, &f.OwnerID, &f.Path, &f.BlobKey, &f.Size, &f.MTime, &f.ETag, &f.MIMEType); err != nil {
