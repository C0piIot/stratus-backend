...
func (r *repo) CreatePlaylist(ctx context.Context, p db.Playlist) (db.Playlist, error) {
	...
	err := r.q.QueryRowContext(ctx, query, p.OwnerID, p.Name, p.Comment, p.Public,
		p.Created.UnixMilli(), p.Changed.UnixMilli()).Scan(&p.ID)
	if err != nil {
...
// LockPlaylist implements db.Playlists. SQLite has no row locks and needs
// none: every transaction here takes the write lock at BEGIN (see New), so
// being inside one is the lock, and this is only the existence check.
func (r *repo) LockPlaylist(ctx context.Context, owner string, id int64) error {
	const query = `SELECT id FROM playlists WHERE owner_id = ? AND id = ?`
...
// SQLite allows 32766 of them; three per row leaves plenty of room.
